...
def algorithm_2(n_nodes, n_edges, edge_list, node_sorted, edge_data, number_colours):
...
C_sorted_by_B = sorted(C, key=lambda x: x[1][1], reverse=True)

# Sort C based on the second values of the elements from A in descending order
C_sorted_by_A = sorted(C_sorted_by_B, key=lambda x: x[0][1], reverse=True)
node_sorted = [elem[0][0] for elem in C_sorted_by_A]

pairs = []
for i in range(len(node_sorted)):
    for j in range(i + 1, len(node_sorted)):
        pairs.append((i, j))
